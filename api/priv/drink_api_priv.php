...
class DrinkAPI extends API
{
	...
	protected function test() {
		return array("result" => true, "message" => "Test Success!", "data" => array("verb" => $this->verb, "args" => $this->args));
	}
		...
		switch($this->verb) {
			...
			*/
			case "drops":
				...
				if (array_key_exists(1, $this->args) && array_key_exists(2, $this->args)) {
					$offset = (int) trim($this->args[2]);
					$sql .= " OFFSET :offset";
	...
	protected function temps() {
		...
		switch($this->verb) {
			/*
			*	Endpoint: temps.machine
			*
			*	Methods:
			*	- machine_one: GET /machine/:machine_id/:limit
			*/
			case "machine":
				// Check method type
				if ($this->method != "GET") {
